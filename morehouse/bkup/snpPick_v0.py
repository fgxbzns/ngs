...
while sam_line_first!='':
	if not sam_line_first.startswith("@"):
		...
		elements = sam_line_first.strip().split()
		try:
			read_ID_first = elements[0].strip()
			rName = elements[2].strip()
			insert_size = int(elements[8].strip())
			
			if (insert_size >= insert_size_lower_bond) and (insert_size <= insert_size_upper_bond):
				...
				quality_score_sequence = elements[10].strip()
				covered_snp = ""
				"""	
				#XA = ""
				try:
					XA = elements[21].strip()
					#print XA
					XA_total += 1
				...
				"""			
				reads_list.append(read(qName, flag, rName, start_position, read_sequence, quality_score_sequence, read_length, covered_snp))
		except:
...
print "total reads", len(reads_list)
...
for snp in snp_list:
	...
	if reads_list[reads_index].start_position <= snp.position:
		if reads_list[reads_index].start_position <= (snp.position - reads_list[reads_index].read_length):
			while reads_list[reads_index].start_position <= (snp.position - reads_list[reads_index].read_length):
				reads_index += 1
				current_read += 1
		while reads_list[reads_index].start_position <= snp.position and reads_list[reads_index].start_position > (snp.position - reads_list[reads_index].read_length):
			current_reads = reads_list[reads_index]
			covered_snp = current_reads.read_sequence[int(snp.position)-int(current_reads.start_position)]
			if not covered_snp == 'N':
				snp.covered_reads_list.append(read(current_reads.qName, current_reads.flag, current_reads.rName, 
				current_reads.start_position, current_reads.read_sequence, current_reads.quality_score_sequence, current_reads.read_length, covered_snp))					
				if covered_snp == 'A':
					snp.allele_dict['A'] += 1
				elif covered_snp == 'T':
					snp.allele_dict['T'] += 1
