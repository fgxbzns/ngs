...
assume last_haplotype_position is the smallest in all three. Make sure the last position in three files is the same.
Limited by current version of hifi.
			...
			if n_alleles == 0 or n_alleles > 2:
				...
				try:
					if geno_A == geno_B:
						...
						if n_alleles == 2:
							if (geno_A == unique_alleles[0] and geno_B == unique_alleles[1]) or (
											geno_A == unique_alleles[1] and geno_B == unique_alleles[0]):
								exist_in_ref = True
...
def load_hap_ref_data_new(ref_file):
	# use strin to load ref
	ref_title_info = ""
	hap_ref_dict = {}

	ref_file_list = ref_file.split(',')

	for infile in ref_file_list:
		ref_file_name = infile
		print ref_file_name
		if ref_title_info == "" and len(hap_ref_dict) == 0:
			ref_title_info, hap_ref_dict = load_raw_data(ref_file_name, raw_data_format_ref)
		else:
	...
	for snp in hap_ref_sorted_list:
		...
		if position <= int(last_seed_position):
			...
			if position in seed_dict:
				...
				hifi_seed_dict[position] = geno_dict[position][0] + " " + geno_dict[position][1] + " " + \
				                           geno_dict[position][2][0]
			#else:
...
def refMerger(haplotype_file, chr_name, remPercent):
	...
	#ref_title_info, hap_ref_dict = load_hap_ref_data(ref_file)
...
def get_args():
	desc = "To generate files for LowDepth"
	...
	parser.add_option("-r", "--ref", type="string", dest="ref_file", help="ref files", default=0)
