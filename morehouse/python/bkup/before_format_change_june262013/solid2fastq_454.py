	...
	for solid_qual_code in solid_qual_list:
		if solid_qual_code == "-1":
			...
			try:		
				...
				print "error in ", solid_qual_code
				print solid_qual
	return fastaq_qual
...
while csfasta_line != "":
	if csfasta_line.startswith(">") and qual_line.startswith(">"):
		if csfasta_line == qual_line:
			...
			reads_complete = True
			title = "@" + csfasta_line + "/1"
			csfasta_line = csfasta_file.readline().strip()
			qual_line = qual_file.readline().strip()
			csfasta_seq = ""
			csfasta_qual = ""
			while not csfasta_line.startswith(">") and csfasta_line != "":
				csfasta_seq += csfasta_line
				csfasta_line = csfasta_file.readline().strip()
			while not qual_line.startswith(">") and qual_line != "":
				csfasta_qual += " " + qual_line
				qual_line = qual_file.readline().strip()		
			#csfasta_line = csfasta_file.readline().strip()
			fastaq_seq = csfasta_seq
			#qual_line = qual_file.readline().strip()
			fastaq_qual = convert_qual(csfasta_qual)
			"""
			if "N" in fastaq_seq:
