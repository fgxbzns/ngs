		...
		if not sam_line_first.startswith("@"):
			...
			while i < read_length_first:
				...
				if current_base_position in hap_std_dict:							
					...
					if (chrName_first == chr_name) and (not covered_snp == 'N') and ((ord(quality_score_symbol)-33) > quality_score_threshold):	# check quality_score
						covered_snp_total_number += 1
						hap_std_dict[current_base_position].covered_reads_list.append(read(qName_first, flag_first, chrName_first, \
					...
					if (not multiple_maping_first)	or (not multiple_maping_second): # keep the pair as long as one read is not multiple mapping
						...
						while i < read_length_first:
							...
							if current_base_position in hap_std_dict:
								...
								if (chrName_first == chr_name) and (not covered_snp == 'N') and ((ord(quality_score_symbol)-33) > quality_score_threshold):	# check quality_score
									covered_snp_total_number += 1
									hap_std_dict[current_base_position].covered_reads_list.append(read(qName_first, flag_first, chrName_first, start_position_first, read_sequence_first, quality_score_sequence_first, read_length_first, covered_snp))					
									for base, value in hap_std_dict[current_base_position].allele_dict.iteritems():
										if base == covered_snp:
											hap_std_dict[current_base_position].allele_dict[base] += 1		
							i += 1
						...
						while i < read_length_second:
							...
							if current_base_position in hap_std_dict:
								...
								if (chrName_second == chr_name) and (not covered_snp == 'N') and ((ord(quality_score_symbol)-33) > quality_score_threshold):
									covered_snp_total_number += 1
									hap_std_dict[current_base_position].covered_reads_list.append(read(qName_second, flag_second, chrName_second, start_position_second, read_sequence_second, quality_score_sequence_second, read_length_second, covered_snp))					
									for base, value in hap_std_dict[current_base_position].allele_dict.iteritems():
										if base == covered_snp:
											hap_std_dict[current_base_position].allele_dict[base] += 1	
							i += 1
	...
	for snp_data in hap_std_sorted_list:
		...
		if len(snp.covered_reads_list) > parameter.depth_threshold:
			...
			for reads in snp.covered_reads_list:
				outputFile_reads.write(reads.qName + "\t" + reads.flag + "\t" + reads.chrName + "\t" + str(reads.start_position) + "\t" \
