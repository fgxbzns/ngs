...
def make_child_HIFI_files(sample_id, raw_ref_title, raw_ref_data):
    ...
    for pos in parameter.pos_list:
        if pos in person_temp.haplotype:
            if person_temp.haplotype[pos][0] != "N" and person_temp.haplotype[pos][0] != "X" or person_temp.haplotype[pos][1] != "N" and person_temp.haplotype[pos][1] != "X":
                print >> output_haplo, parameter.rsID_dict[pos] + "\t" + str(pos) + "\t" + person_temp.haplotype[pos][0] + "\t" + person_temp.haplotype[pos][1]
    output_haplo.close()
