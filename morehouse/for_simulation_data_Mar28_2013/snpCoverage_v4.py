...
# check snp coverage
# program runs fast without preprocess the reads
...
def searchReads(snp, reads_list):
	escape = False
	temp_reads_list = reads_list
	temp_reads_list = removeUncoverdReads(snp, temp_reads_list)
	#print "tem_reads_list size ", len(temp_reads_list)
	for reads in temp_reads_list:	
		print "snp.position", snp.position
		print "reads.start_position", reads.start_position
		snp.covered_reads_list.append(reads)			
		base = reads.read_sequence[int(snp.position)-int(reads.start_position)-1]
		reads.base = base   #????
		if base == 'A':
			snp.allele_dict['A'] += 1
		elif base == 'T':
...
# process reads list to keep the reads that cover the current snp position
# this makes a copy of the lsit and is slow
def removeUncoverdReads(snp, reads_list):
	reads_list = [reads for reads in reads_list if (reads.start_position <= snp.position and reads.start_position > (snp.position-100))]
	return reads_list

# remove the reads with position smaller than snp position - read length
# this changes the original list and is fast
def preProcessReadsList(snp, reads_list):
	reads_list[:] = [reads for reads in reads_list if (reads.start_position > (snp.position-100))]
	return reads_list
...
if sam_file == "null":
	...
	#sam_file = "NA12878_hg18ch6_A_4.0x_0.01er.sam"
...
outputFile_sorted_sam_processed = open(currentPath + sam_file_name + "_sorted_sam_processed.txt", "w")
outputFile_sorted_sam_processed.write("qname \t flag \t rname \t position \t sequence Depth \n")
...
for line in inputFile_hap:
	#line = inputFile_hap.readline().strip()
...
for line in inputFile_sam:
	if not line.startswith("@"):
		...
		try:
			...
			print line
		#if not rName == "*":  # remove reads without chr name
...
temp_list = []
while i < 5000:
	temp_list.append(reads_list[i].start_position)
	i+=1
print temp_list
"""
...
#print snp_list[0].position
print "haplotye list size is: ", len(snp_list)
print "reads list size is: ", len(reads_list)

for reads in reads_list:
	outputFile_sorted_sam_processed.write(reads.qName + "\t" + reads.flag + "\t" + reads.rName + "\t" + str(reads.start_position) + "\t" + reads.read_sequence + "\n")

"""
print snp_list[1].position
snp = searchReads(snp_list[1], reads_list)
print "current reads list size: ", len(reads_list)
if len(snp.covered_reads_list) > 0:
		print "sample ", len(snp.covered_reads_list)
print "current reads list size: ", len(reads_list)
"""

# preprocess the reads list for first snp. Might be useful for certain cases
#reads_list = preProcessReadsList(snp_list[0], reads_list)

# preprocess the reads list for each snp, then remove uncoverd reads 
#snp_list = snp_list[:500]	
...
for snp in snp_list:
	...
	if reads_list[reads_index].start_position <= snp.position:
		if reads_list[reads_index].start_position <= (snp.position - 100):				# 100 reads_length  need to adjust
			while reads_list[reads_index].start_position <= (snp.position - 100):
				reads_index += 1
				current_read += 1
		#if reads_list[reads_index].start_position <= snp.position and reads_list[reads_index].start_position > (snp.position - 100):
		if True:
			while reads_list[reads_index].start_position <= snp.position and reads_list[reads_index].start_position > (snp.position - 100):
				current_reads = reads_list[reads_index]
				base = current_reads.read_sequence[int(snp.position)-int(current_reads.start_position)]
				# remove base N ??? remove or not? 
				if not base == 'N':
				#if True:
					...
					if base == 'A':
						...
						snp.allele_dict['G'] += 1									
				reads_index += 1
					#print snp.allele_dict
						
"""			
for snp in snp_list:
	if snp.allele_dict['A'] > 0 or snp.allele_dict['T'] > 0:
		print snp.allele_dict	
"""	
	
for snp in snp_list:
	if len(snp.covered_reads_list) > 0:
		#print len(snp.covered_reads_list)
...
outputFile_sorted_sam_processed.close()
